  ...
  return (
    <section className="w-full py-12">
      <div className="max-w-[1000px] px-12 ">
        {/* Heading */}
        <h2 className="text-[28px] lg:text-[36px] font-medium text-[#6E26CB] mb-6">
          {heading}
        </h2>
        ...
        <ol className="list-decimal lg:pl-15 space-y-5 text-[#3A3A3A] text-justify">
          {terms.map((term, index) => (
            <li key={index}>
              <p className="font-normal text-[24px] lg:text-[28px] mb-1">
                {term.title}
              </p>

              {term.description && (
                <p className="text:[16px] lg:text-[20px] leading-[22px] text-[#6B6B6B]">
                  {term.description}
                </p>
              )}
