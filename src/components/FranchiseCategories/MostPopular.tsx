...
export default function MostPopular({ categoryId }: Props) {
  const { services, fetchMostPopularServiceByCategory, loading, error } = useMostPopularServiceByCategory();
  ...
  if (loading)
    return <p className="text-center py-10">Loading most popular services...</p>;
  if (error) return <p className="text-center py-10 text-red-500">{error}</p>;
  if (services.length === 0)
    return <p className="text-center py-10">No most popular services found.</p>;
