...
// export default function DetailsStep({ onNext }: DetailsStepProps) {
//     const { service, loading: serviceLoading, error: serviceError, fetchServiceDetails } = useServiceDetails();
//     const { reviewServices, fetchReviews } = useReview();
//     const params = useParams();
//     const serviceId = params.id as string;
...
//     const { selectedPackage } = useCheckout();
//     const { services, fetchCommission, loading: commissionLoading } = useCommission();

//     // Fetch all data when component mounts
//     useEffect(() => {
//         if (!serviceId || dataFetchedRef.current) return;
...
//         // Safety timeout - force show UI after 3 seconds even if still loading
//         const timeoutId = setTimeout(() => {
//             setForceShowUI(true);
//         }, 3000);
        
//         return () => clearTimeout(timeoutId);
//     }, [serviceId]); // Remove functions from dependencies
...
//     // Check if we have the essential data to render
//     const hasEssentialData = packageToUse && (service || commission);

//     // Show loading state only if we don't have data and haven't timed out
//     if ((serviceLoading || commissionLoading) && !hasEssentialData && !forceShowUI) {
//         return (
...
//     if (!packageToUse && !forceShowUI) {
//         return (
//             <div className="w-full flex items-center justify-center min-h-[400px]">
//                 <div className="text-center text-gray-500">
//                     <p>No package available for this service</p>
//                 </div>
//             </div>
//         );
//     }
...
//     // ===== PAYMENT CALCULATION =====
//     const listingPrice = packageToUse?.price || paymentData.listingPrice;
//     const serviceDiscount = packageToUse?.discount || paymentData.serviceDiscount;
//     const priceAfterServiceDiscount = listingPrice - serviceDiscount;
//     const couponDiscount = paymentData.couponDiscount;
...
//     const gstPercent = service?.gst ?? 18;
//     const gstAmount = (priceAfterCoupon * gstPercent) / 100;
//     const platformFee = commission?.platformFee ?? paymentData.platformFee;
//     const assuranceFee = commission?.assurityfee ?? paymentData.assuranceFee;
...
//                             <div className="relative">
//                                 <img 
//                                     src={service?.images?.[0] || serviceCardData.image} 
//                                     className="md:w-[300px] md:h-[200px] lg:w-[455px] lg:h-[295px] object-cover" 
...
//                                     </span>
//                                 )}
//                                 <div className="absolute bottom-2 right-2 bg-blue-600 md:w-[47px] md:h-[31px] lg:w-[67px] lg:h-[43px] text-white md:text-[10px] lg:text-[14px] px-2 py-1 rounded-md flex items-center justify-center">
//                                     ⭐ {reviewServices?.averageRating || serviceCardData.rating}
//                                 </div>
//                             </div>
//                             <div className="p-4 space-y-3">
//                                 <div className="flex items-center justify-between">
//                                     <div>
//                                         <h3 className="font-semibold md:text-[15px] lg:text-[20px]">
//                                             {service?.serviceName || serviceCardData.title}
//                                         </h3>
//                                         <p className="lg:text-[16px] md:text-[12px] text-gray-500">
//                                            {service?.category?.name || serviceCardData.category}
//                                         </p>
//                                     </div>
//                                     <div className="flex flex-col">
//                                         <p className="lg:text-[10px] md:text-[8px] text-[#4A2E82] font-medium whitespace-normal breakwords">
//                                             -EARN UP TO-
//                                         </p>
//                                         <span className="text-[#2CB140] lg:text-[14px] md:text-[10px] text-center">
//                                             {serviceCardData.rewardText}
//                                         </span>
//                                     </div>
//                                 </div>
...
//                             <div className="flex gap-4 lg:pl-6">
//                                 <button
//                                     className="border border-blue-600 text-blue-600 px-4 py-2 rounded-lg md:text-[12px] whitespace-nowrap lg:text-[14px]"
//                                     onClick={() => setOpenSidebar(true)}
//                                 >
//                                     My Customer
//                                 </button>
//                                 <button
//                                     className="bg-blue-600 text-white px-4 py-2 rounded-lg cursor-pointer md:text-[12px] flex flex-row items-center lg:text-[14px] gap-2"
//                                     onClick={() => setOpenAddCustomers(true)}
//                                 >
//                                     <PlusCircle className="w-4 h-4 flex shrink-0" />Add New Customer
//                                 </button>
...
//                     </h3>
//                     <div className="space-y-3 md:text-[12px] lg:text-[16px]">
//                         <Row label="Listing Price" value={`₹ ${listingPrice.toFixed(2)}`} />
//                         <Row label="Service Discount" value={`- ₹ ${serviceDiscount.toFixed(2)}`} valueClass="text-red-500" />
//                         <Row label="Price After Discount" value={`₹ ${priceAfterServiceDiscount.toFixed(2)}`} />
//                         <Row label="Coupon Discount" value={`- ₹ ${couponDiscount.toFixed(2)}`} valueClass="text-red-500" />
//                         <Row label={`Service GST (${gstPercent}%)`} value={`+ ₹ ${gstAmount.toFixed(2)}`} valueClass="text-green-600" />
...
//                         disabled={!isTermsAccepted}
//                         className={`px-10 py-3 rounded-lg md:text-[15px] lg:text-[20px] font-medium
//                             ${isTermsAccepted ? "bg-blue-600 text-white cursor-pointer hover:bg-blue-700" : "bg-gray-400 text-white cursor-not-allowed"}
//                         `}
//                         onClick={handleProceed}
//                     >
//                         Proceed To Pay
...
//                     <div className="space-y-3 md:text-[12px] lg:text-[16px]">
//                         <Row label="Listing Price" value={`₹ ${paymentData.listingPrice.toFixed(2)}`} />
//                         <Row label="Service Discount (0%)" value={`- ₹ ${paymentData.serviceDiscount.toFixed(2)}`} valueClass="text-red-500" />
//                         <Row label="Price After Discount" value={`- ₹ ${packageToUse.discountedPrice.toFixed(2)}`} />
//                         <Row label="Coupon Discount (50%)" value={`- ₹ ${paymentData.couponDiscount.toFixed(2)}`} valueClass="text-red-500" />
//                         <Row label="Service GST (18%)" value={`+ ₹ ${paymentData.gst.toFixed(2)}`} valueClass="text-green-600" />
//                         <Row label="Platform Fee (20)" value={`+ ₹ ${platformFee.toFixed(2)}`} valueClass="text-green-600" />
...
//                 <div className="flex items-center justify-between bg-blue-50 px-4 py-3 rounded-lg">
//                     <p className="text-blue-600 font-semibold md:text-[15px] lg:text-[20px]">Grand Total</p>
//                     <p className="text-blue-600 font-semibold md;text-[15px] lg:text-[20px]">₹ {paymentData.grandTotal}</p>
//                 </div>
...
//                         disabled={!isTermsAccepted}
//                         className={`px-10 py-3 rounded-lg md:text-[15px] lg:text-[20px] font-medium
//                             ${isTermsAccepted ? "bg-blue-600 text-white cursor-pointer" : "bg-gray-400 text-white cursor-not-allowed"}
//                         `}
//                         onClick={handleProceed}
//                     >
//                         Proceed To Pay
