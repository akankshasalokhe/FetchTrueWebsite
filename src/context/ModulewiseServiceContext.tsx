...
interface ModulewiseServiceContextType {
  services: Service[];
  loading: boolean;
  error: string | null;
...
const ModulewiseServiceContext =
  createContext<ModulewiseServiceContextType | undefined>(undefined);

/* ================= PROVIDER ================= */

export const ModulewiseServiceProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  const fetchServicesByModule = useCallback(async (moduleId: string) => {
    try {
      setLoading(true);
      setError(null);

      const res = await axios.get(
        `https://api.fetchtrue.com/api/service/module?moduleId=${moduleId}`
      );
  ...
  return (
    <ModulewiseServiceContext.Provider
      value={{ services, loading, error, fetchServicesByModule }}
    >
      {children}
    </ModulewiseServiceContext.Provider>
  );
...
export const useModulewiseServices = () => {
  const context = useContext(ModulewiseServiceContext);
  if (!context) {
    throw new Error(
      "useModulewiseServices must be used within ModulewiseServiceProvider"
    );
  }
  return context;
};
